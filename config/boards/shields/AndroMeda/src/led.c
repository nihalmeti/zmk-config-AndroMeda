...
static int led_keylock_listener_cb(const zmk_event_t *eh) {
    ...
    if (flags & capsBit) {
        led_set(led_dev, DT_NODE_CHILD_IDX(DT_ALIAS(led_caps)), 1);  // Turn on Caps Lock LED
    } else {
        led_set(led_dev, DT_NODE_CHILD_IDX(DT_ALIAS(led_caps)), 0);  // Turn off Caps Lock LED
    }
